...
import { useCSVImportSalesforce, useSettings } from "@cadence-frontend/data-access";
...
function CsvColumnMap() {
	...
	} = csvImportSalesforceDataAccess;

	const user = useRecoilValue(userInfo);

	const { fetchSfMap, fetchSfMapLoading, fetchLeadFieldsLoading } = useSettings({
		role: user.role,
		enabled: false,
	});
	...
	const handleDragEnd = e => {
		...
		if (
			...
			// if (
			// 	draggableId.name.toLowerCase().includes("salesforce owner id") &&
			// 	destinationDroppableId !== "Salesforce Owner ID"
			// )
			// 	return;
			// if (
			// 	destinationDroppableId === "Salesforce Owner ID" &&
			...
			if (
				draggableId.name === "Salesforce Owner ID" ||
				destinationDroppableId === "Salesforce Owner ID"
			)
				return;
