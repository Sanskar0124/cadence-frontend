...
function CsvLeadMapping() {
	...
	const fetchSfFieldMap = () => {
		...
		fetchSfMap(null, {
			onSuccess: ringoverFieldsFromServer => {
				...
				Object.keys(ringoverFieldsFromServer?.account_map).forEach(f => {
					if (f === "name" || f === "phone_number") {
						arr.push(`company_${f}`);
					} else {
						arr.push(f);
					}
	...
	const handleDragEnd = e => {
		...
		if (
			...
			// if (
			// 	draggableId.name.toLowerCase().includes("bullhorn owner id") &&
			// 	destinationDroppableId !== "Bullhorn Owner ID"
			// )
			// 	return;
			// if (
			// 	destinationDroppableId === "Bullhorn Owner ID" &&
			...
			if (
				draggableId.name === "Bullhorn Owner ID" ||
				destinationDroppableId === "Bullhorn Owner ID"
			)
				return;
	...
	const handleImportLeads = () => {
		...
		navigate(
			`/cadence-import?import_from=csv&import_type=create_lead&type=${type}&cadence_id=${cadenceId}&cadence_name=${cadenceName}`
		);
	};
		...
		if (leadField.length && originalContactFields.length && extractedColumns?.length) {
			...
			let fields = getRequiredField([
				...[
					...
					{
						name: "bullhorn_owner_id",
						label: "Bullhorn Owner ID",
						type: "string",
					},
				],
				...leadField?.map(field => ({
								...
								POTENTIAL_FIELDS[field.name]?.forEach(label => {
									...
									if (res) {
										defaultDisplayedColumns.splice(
											defaultDisplayedColumns.indexOf(res),
											1
										);
										newField = {
											...field,
