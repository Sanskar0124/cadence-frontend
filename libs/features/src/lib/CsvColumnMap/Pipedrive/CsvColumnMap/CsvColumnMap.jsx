...
import { useCSVImportPipedrive, useSettings } from "@cadence-frontend/data-access";
...
function CsvColumnMap() {
	...
	const fetchSfFieldMap = () => {
		fetchSfMap(null, {
			onSuccess: ringoverFieldsFromServer => {
				...
				Object.keys(ringoverFieldsFromServer?.organization_map).forEach(f => {
	...
	const handleDragEnd = e => {
		...
		if (
			...
			// if (
			// 	draggableId.name.toLowerCase().includes("pipedrive owner id") &&
			// 	destinationDroppableId !== "Pipedrive Owner ID"
			// )
			// 	return;
			// if (
			// 	destinationDroppableId === "Pipedrive Owner ID" &&
			...
			if (
				draggableId.name === "Pipedrive Owner ID" ||
				destinationDroppableId === "Pipedrive Owner ID"
			)
				return;
	...
	const handleImportLeads = () => {
		...
		navigate(
			`/cadence-import?type=create_lead&import_type=${type}&cadence_id=${cadenceId}&cadence_name=${cadenceName}`
		);
	};
	...
	useEffect(() => {
		if (leadField.length && extractedColumns?.length) {
			...
			let fields = getRequiredField([
				...[
					...
					{
						name: "pipedrive_owner_id",
						label: "Pipedrive Owner ID",
						type: "string",
					},
				],
				...leadField?.map(field => ({
