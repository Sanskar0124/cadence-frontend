...
const LeadDetailsModal = ({ modal, setModal, type }) => {
		...
		<Modal isModal={modal} onClose={handleClose} showCloseButton>
			<div className={styles.leadDetailsModal}>
				...
				<div className={styles.heading}>
					{type.toString() === "lead" || type.toString() === "lead_list" ? (
						<h3>{CADENCE_TRANSLATION?.LEAD_DETAILS[user?.language?.toUpperCase()]}</h3>
					) : type.toString() === "contact" || type.toString() === "contact_list" ? (
				...
				<div className={styles.main}>
					<div className={styles.col1}>
						...
						<div className={styles.group}>
							...
							<div className={styles.text}>{modal?.Account?.size ?? <span>NA</span>}</div>
						</div>
						<div className={styles.group}>
							<label>
						...
						<div className={styles.group}>
							<label>
								{TASKS_TRANSLATION.COMPANY_ZIP_CODE[user?.language?.toUpperCase()]}
							</label>
							<div className={styles.text}>
								{modal?.Account?.zipcode ?? modal?.Account?.zip_code ?? <span>NA</span>}
							</div>
						</div>
						<div className={styles.group}>
						...
						{modal?.phone_numbers?.length > 0 &&
							modal?.phone_numbers?.map(pn => (
								<div className={styles.group} key={pn.lpn_id}>
									<label>{pn.type}</label>
									<div className={styles.text}>
										{pn.phone_number?.toString().length ? (
											pn.phone_number
										) : (
											<span>NA</span>
										)}
