...
const SelectCadence = ({ cadenceSelected, setCadenceSelected, list, checkedLeads }) => {
	...
	return (
		<div className={styles.selectCadence} ref={buttonRef}>
			<ThemedButton
				...
				disabled={!list?.length && !checkedLeads?.length}
			>
				<span>
					<Cadences />
				</span>
				...
				<div className={`${styles.dropdown} ${isDropdown ? styles.open : ""}`}>
					...
					{cadenceLoading ? (
						...
						<div className={styles.noCadence}>
							<CadenceEmpty size="11rem" />
							<h4>
								{CADENCE_TRANSLATION.NO_CADENCE_TO_IMPORT[
									user?.language?.toUpperCase()
								].replace(
									"{cadence_type}",
									CADENCE_TRANSLATION[tab?.toUpperCase()][
										user?.language?.toUpperCase()
									]?.toLowerCase()
								)}
							</h4>
