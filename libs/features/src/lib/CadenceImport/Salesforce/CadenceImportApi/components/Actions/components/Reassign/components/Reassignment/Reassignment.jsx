...
const Reassignment = ({ modal, setModal, list, checkedLeads, leadType }) => {
	...
	useEffect(() => {
		if (toggle && users.length > 0) {
			let leadsNo = filteredList.length;
	...
	const handleSubmit = e => {
		...
		if (leadType == TYPES.LEAD && leadsLeft > 0) {
			addError({
				text:
					leadsLeft === 1
		...
		if (leadType == TYPES.CONTACT && contactsLeft > 0) {
			addError({
				text:
					contactsLeft === 1
			...
			<div className={styles.body}>
				{openedPeopleView ? (
					...
					<div className={styles.reassign}>
						<div className={styles.inputGroup} ref={usersDropdownRef}>
						...
						<div className={styles.reassign}>
							<div className={styles.header}>
								...
								<div className={styles.distnStatus}>
									{leadType == TYPES.LEAD && leadsLeft > 0 && (
										<HighlightBox
											height={26}
											fontWeight={700}
											borderRadius={5}
									...
									{leadType == TYPES.CONTACT && contactsLeft > 0 && (
										<HighlightBox
											height={26}
											fontWeight={700}
											borderRadius={5}
						...
						<div className={styles.inputGroup}>
							...
							{leadType === TYPES.CONTACT_LIST || leadType === TYPES.CONTACT ? (
								<>
									<Select
										options={Object.freeze({
											[COMPANY_CONTACT_REASSIGNMENT_OPTIONS.CONTACT_ACCOUNT_AND_OTHER_CONTACTS]:
