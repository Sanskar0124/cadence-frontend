...
const CadenceImport = () => {
	...
	//refs
	const loader = useRef({
		ids: { loader1: uuidv4(), loader2: "", loader3: uuidv4() },
		length: 0,
	});
	...
	const handlePreviewResponse = data => {
		if (data?.error) {
			...
			if (getLeadsExcludingError(data?.data?.previewLeads).length > 1000) {
				setLeadsLimitModal(true);
				setCheckedLeads(getLeadsExcludingError(data?.data?.previewLeads).slice(0, 1000));
			} else setCheckedLeads(getLeadsExcludingError(data?.data?.previewLeads));
		}
	...
	const handleAddToCadence = (lead, stopPreviousCadences) => {
		loader.current = {
			ids: {
				loader1: loader?.current?.ids?.loader1,
			},
			length: list.filter(
				lead =>
	...
	useEffect(() => {
		...
		//Preview Response
		socket3.current.on("cadence-import-response", data => handlePreviewResponse(data));
		socket3.current.on("msg", data => handleRoomJoined(data));
						...
						<Tooltip text="Column mapping" className={styles.csvTooltip}>
							<ThemedButton
								...
								onClick={() =>
									cadence_id
										? navigate(
												`/import-csv?type=sheets&cadence_id=${cadence_id}&cadence_name=${cadence_name}`
										  )
										: navigate("/cadence")
								}
								width="fit-content"
