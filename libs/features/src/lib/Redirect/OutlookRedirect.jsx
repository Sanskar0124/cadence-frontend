...
import { useOutlook } from "@cadence-frontend/data-access";
...
const OutlookRedirect = () => {
	...
	const { redirect, redirectLoading } = useOutlook();
	...
	useEffect(() => {
		redirect(auth_code, {
			onSuccess: () => goTo(sessionStorage.getItem("redirectAfterOutlookAuth")),
			...
			<div className={styles.redirectPage}>
				...
				{error && (
					<>
						...
						<ThemedButton
							onClick={() => goTo(sessionStorage.getItem("redirectAfterOutlookAuth"))}
