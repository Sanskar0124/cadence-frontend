...
const DNDView = ({
	...
	useEffect(() => {
		setDisableNext(
			ringoverMatchFieldsLoading || updateRingoverMatchFieldsLoadingExtension
		);
	}, [ringoverMatchFieldsLoading, updateRingoverMatchFieldsLoadingExtension]);
	...
	const fetchItFields = () => {
		...
		fetchCompanySellsyFieldsMutate(VIEWS.COMPANY, {
			onSuccess: itFieldsFromServer => {
	...
	useEffect(() => {
		...
		console.log(originalItFields, "originalItFields");
	...
	const updateRingoverFields = ({ cb }) => {
		...
		if (warnings[VIEWS.COMPANY].length + warnings[VIEWS.CONTACT].length === 0) {
			...
			updateRingoverMatchFieldsExtension(body, {
				onError: err => {
					addError({
						text: err?.message,
