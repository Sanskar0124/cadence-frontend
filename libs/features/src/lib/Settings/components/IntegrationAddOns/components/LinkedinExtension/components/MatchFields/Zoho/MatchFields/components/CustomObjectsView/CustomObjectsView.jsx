...
const CustomObjectsView = ({
	...
	useEffect(() => {
		setAvailableSFFields({
			[VIEWS?.LEAD]: originalSFFields[VIEWS.LEAD]
				?.filter(oriField => {
					return (
						form?.filter(
							formElement =>
								formElement.zoho_field === oriField.name &&
								formElement.zoho_endpoint === "lead"
						).length === 0
					);
				})
				?.filter(f => f.types !== "jsonobject"),
			[VIEWS?.ACCOUNT]: originalSFFields[VIEWS.ACCOUNT]
				?.filter(oriField => {
					return (
						form?.filter(
							formElement =>
								formElement.zoho_field === oriField.name &&
								formElement.zoho_endpoint === "account"
						).length === 0
					);
				})
				?.filter(f => f.types !== "jsonobject"),
			[VIEWS?.CONTACT]: originalSFFields[VIEWS.CONTACT]
				?.filter(oriField => {
					return (
						form?.filter(
							formElement =>
								formElement.zoho_field === oriField.name &&
								formElement.zoho_endpoint === "contact"
						).length === 0
					);
				})
				?.filter(f => f.types !== "jsonobject"),
