...
const DNDView = ({
	...
	useEffect(() => {
		console.log("ringoverFields", ringoverFields);
	}, [ringoverFields]);

	useEffect(() => {
		console.log("originalSFFields", originalSFFields);
	}, [originalSFFields]);
	...
	const handleDragEnd = e => {
		...
		console.log("DRAGGABLE", draggableData);

		// picklist should only go to picklist or any
		if (
			destinationData.type !== "sf" &&
			draggableData.type === "picklist" &&
			!["picklist", "any"].includes(destinationData.type[0]) &&
			!["picklist", "any"].includes(destinationData.type[1])
		) {
		...
		if (sourceData.type === "sf") {
			...
			if (
				...
				draggableData.type !== "string" // picklist can have picklist value or string value
			) {
