			...
			onSuccess: leadFields => {
				setOriginalSFFields(prev => ({
					...
					[VIEWS.LEAD]: leadFields
						...
						.map((field, i) => ({ index: i, ...field, integration_endpoint: "lead" })),
				}));
			},
		});
			...
			onSuccess: accountFields => {
				setOriginalSFFields(prev => ({
					...
					[VIEWS.ACCOUNT]: accountFields
						?.sort((a, b) => a.label.localeCompare(b.label))
						.filter(item => item.type !== "address")
						.map((field, i) => ({ index: i, ...field, integration_endpoint: "account" })),
				}));
			},
		});
			...
			onSuccess: contactFields => {
				setOriginalSFFields(prev => ({
					...
					[VIEWS.CONTACT]: contactFields
						?.sort((a, b) => a.label.localeCompare(b.label))
						.filter(item => item.type !== "address")
						.map((field, i) => ({ index: i, ...field, integration_endpoint: "contact" })),
				}));
			},
		});
	};
		...
		setAvailableSFFields({
			[VIEWS?.LEAD]: originalSFFields[VIEWS.LEAD]?.filter(oriField => {
				return (
					form?.filter(
						formElement =>
							formElement.integration_field === oriField.name &&
							formElement.integration_endpoint === "lead"
					).length === 0
				);
			}),
			[VIEWS?.ACCOUNT]: originalSFFields[VIEWS.ACCOUNT]?.filter(oriField => {
				return (
					form?.filter(
						formElement =>
							formElement.integration_field === oriField.name &&
							formElement.integration_endpoint === "account"
					).length === 0
				);
			}),
			[VIEWS?.CONTACT]: originalSFFields[VIEWS.CONTACT]?.filter(oriField => {
				return (
					form?.filter(
						formElement =>
							formElement.integration_field === oriField.name &&
							formElement.integration_endpoint === "contact"
					).length === 0
				);
			}),
		});
		...
		<div>
			...
			<div className={styles.body}>
				<div className={styles.input}>
					<Label required>Text displayed</Label>
				...
				<div className={styles.info}>
					* Please open the form builder to create your own custom form.
				</div>
			</div>
			<FormBuilderModal
