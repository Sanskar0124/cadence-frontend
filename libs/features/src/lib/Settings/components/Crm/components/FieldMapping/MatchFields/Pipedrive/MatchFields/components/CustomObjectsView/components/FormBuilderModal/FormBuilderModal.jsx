...
const FormBuilderModal = ({
	...
	const handleDrag = result => {
		...
		if (result.source.droppableId === "pd") {
			...
			let obj = {
				...
				pipedrive_field: data.name,
				pipedrive_endpoint: data.pipedrive_endpoint,
				pipedrive_label: data.label,
			...
			// }
			else if (data.type === "email" || data.type === "phone" || data.name === "email") {
				obj.type = "input_select";
				obj.input_type = data.type;
			} else {
				obj.type = "input_box";
				obj.input_type = data.type;
			}
			if (obj.type === "input_select") {
				if (form.find(item => item?.position?.row === row && item.type !== "") != null) {
					return addError({
						text: "Phone or email type fields cannot be droppped in same row with other fields.",
					});
				}
			} else {
				if (
					form.find(
						item => item?.position?.row === row && item.type === "input_select"
					) != null
				)
					return addError({
						text: "You cannot drop other fields if phone or email type field is present in a row.",
					});
			}
		...
		} else if (result.destination.droppableId === "pd") {
			...
			if (form[initialIndex].type === "input_select") {
				if (
					finalRow !== initialRow &&
					form.find(item => item?.position?.row === finalRow && item.type !== "") != null
				) {
					return addError({
						text: "Phone or email type fields cannot be droppped in same row with other fields.",
					});
				...
				if (
					form.find(
						item => item?.position?.row === finalRow && item.type === "input_select"
					) != null
				)
					return addError({
						text: "You cannot drop other fields if phone or email type field is present in a row.",
			...
			setForm(prev => {
				...
				// if (prev[finalIndex].type === "") {
				// 	let mincol = finalCol;
				// 	form
				// 		.filter(item => item.type === "" && item.position.row === finalRow)
				// 		.forEach(item => {
				// 			if (item.position.column < mincol) {
				// 				mincol = item.position.column;
				// 			}
				// 		});
				// 	prev.find(
				// 		item => item.position.column === mincol && item.position.row === finalRow
				// 	).position.column = finalCol;
				// 	prev[initialIndex].position.column = mincol;

				// 	let obj = {
				// 		type: "",
				// 		position: { row: initialRow, column: MAX_NUMBER_OF_COL - 1 },
				// 	};
				...
				// 					item =>
				// 						!(
				// 							item.position.row === initialRow &&
				// 							item.position.column === initialCol
				// 						)
				// 				)
				// 				.map(item => {
				// 					if (
				...
				// 						return {
				// 							...item,
				// 							position: { row: initialRow, column: item.position.column - 1 },
				// 						};
				// 					} else {
				// 						return item;
				// 					}
			...
			<div className={styles.main}>
				...
				<DragDropContext onDragEnd={result => handleDrag(result)}>
					<div className={styles.body}>
						...
						/>
						<PipedriveFields availablePDFields={availablePDFields} loading={loading} />
					</div>
				</DragDropContext>
			</div>
		</Modal>
