import styles from "./Sellsy.module.scss";
...
import { SellsyFieldsPlaceholder } from "../Placeholders/Placeholders";

const SellsyFields = ({ availableSellsyFields, loading }) => {
	const [searchValue, setSearchValue] = useState("");
	const [viewSellsyFieldsFor, setViewSellsyFieldsFor] = useState(VIEWS.COMPANY);
	return (
			...
			<div className={styles.header}>
				<Title className={styles.title} size="1.2rem">
					...
					<ThemedButton
						...
						width="10px"
					>
						{availableSellsyFields[viewSellsyFieldsFor]?.length}
					</ThemedButton>
				</Title>
				...
				<div className={styles.tabNavBox}>
					<TabNavSlider
						...
						value={viewSellsyFieldsFor}
						setValue={setViewSellsyFieldsFor}
						...
						{provided => (
							...
							>
								{ALPHABETS.map((alph, j) => {
									return availableSellsyFields[viewSellsyFieldsFor]
										?.filter(field =>
										...
										<div className={styles.subSection}>
											...
											<div className={styles.subSectionFields}>
												{availableSellsyFields[viewSellsyFieldsFor]
													?.filter(field =>
														field.label.toLowerCase().includes(searchValue.toLowerCase())
													)
