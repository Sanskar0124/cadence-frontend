...
const QuickView = ({
	...
	const [bullhornFileld, setBullhornFileld] = useState(FIELDS);
		...
		<div className={`${styles.quickView} `}>
			...
			<div className={styles.header}>
				<ThemedButton
					...
					disabled={INTEGRATION_TYPE.BULLHORN === user?.integration_type}
			...
			{testFieldsCustomObjectModal && (
				<TestFieldsCustomObjectModal
					...
					fields={bullhornFileld}
					setFields={setBullhornFileld}
					url={url}
					setUrl={setUrl}
				/>
			)}

			{Object?.keys(bullhornFileld[buildFormFor])?.length !== 0 && (
