import { useState, useEffect } from "react";
import styles from "./FetchedQuickView.module.scss";

import {
	Briefcase,
	Building,
	Email,
	Home,
	LinkBox,
	LinkedinBox,
	Location,
	ZoomInfo,
} from "@cadence-frontend/icons";
import { Colors } from "@cadence-frontend/utils";
import { VIEWS, LEADS_CURRENT_VIEWS } from "../../../../../constants";
import { conditionForLead, conditionForCandidate } from "../constants";

//constants
const designField = ({
	text = "Error",
	size = "1rem",
	color = "black",
	className = "",
	...rest
}) => {
	return (
		<div
			className={`${styles.error} ${className}`}
			{...rest}
			style={{ color }}
			title={text}
		>
			{text}
		</div>
	);
};

const FetchedQuickView = ({
	fieldsResult,
	currentView,
	isOnboarding,
	leadsCurrentView,
	current_view,
}) => {
	const Render = (backendField, className) => {
		const item = fieldsResult.filter(item => item.backendField === backendField)?.[0];
		//diff view case
		if (!item) return null;
		//missed field case
		if (item.value.name === "")
			return designField({ text: item.label, color: Colors.mainPurple, className });
		// unmatched field case
		else if (item.value.name !== "" && !item.retrievedValue)
			return designField({ text: item.label, color: Colors.redShade2, className });
		//field retrieved case
		else if (item.value.name !== "" && item.retrievedValue)
			return designField({ text: item.retrievedValue, color: Colors.black, className });
	};
	const [linkedinUrl, setLinkedinUrl] = useState(null);
	const [companyUrl, setCompanyUrl] = useState(null);
	const [zoomInfo, setZoomInfo] = useState(null);
	const [showAllEmails, setShowEmails] = useState(false);
	const [showAllPhones, setShowAllPhones] = useState(false);

	useEffect(() => {
		if (fieldsResult) {
			setLinkedinUrl(
				fieldsResult.filter(
					item =>
						item.backendField === "linkedin_url" &&
						item.retrievedValue &&
						item.value.name !== ""
				)?.[0]?.retrievedValue
			);
			setCompanyUrl(
				fieldsResult.filter(
					item =>
						item.backendField === "url" && item.retrievedValue && item.value.name !== ""
				)?.[0]?.retrievedValue
			);
			setZoomInfo(
				fieldsResult.filter(
					item =>
						item.backendField === "zoom_info" &&
						item.retrievedValue &&
						item.value.name !== ""
				)?.[0]?.retrievedValue
			);
		}
	}, [fieldsResult]);

	return (
		<div className={styles.fetchedQuickView}>
			<div className={styles.body}>
				<div className={styles.integrationStatus}>
					{Render("integration_status", styles.integrationStatusButton)}
				</div>
				<div className={styles.redirections}>
					<div
						className={styles.hoverContainer}
						style={{
							cursor: linkedinUrl && linkedinUrl !== "Not set" ? "pointer" : "default",
						}}
						onClick={() =>
							linkedinUrl && linkedinUrl !== "Not set"
								? window.open(linkedinUrl, "_blank")
								: null
						}
					>
						<LinkedinBox
							size="1.8rem"
							color={
								fieldsResult.filter(
									item =>
										item.backendField === "linkedin_url" &&
										!item.retrievedValue &&
										item.value.name !== ""
								).length
									? Colors.redShade3
									: "#0077B7"
							}
						/>
					</div>
					<div
						className={styles.hoverContainer}
						style={{
							cursor: companyUrl && companyUrl !== "Not set" ? "pointer" : "default",
						}}
						onClick={() =>
							companyUrl && companyUrl !== "Not set"
								? window.open(companyUrl, "_blank")
								: null
						}
					>
						<LinkBox
							size="1.8rem"
							color={
								fieldsResult.filter(
									item =>
										item.backendField === "company_url" &&
										!item.retrievedValue &&
										item.value.name !== ""
								).length
									? Colors.redShade3
									: Colors.lightBlue
							}
						/>
					</div>
					{/* <div
						className={styles.hoverContainer}
						style={{
							cursor: zoomInfo && zoomInfo !== "Not set" ? "pointer" : "default",
						}}
						onClick={() =>
							zoomInfo && zoomInfo !== "Not set" ? window.open(zoomInfo, "_blank") : null
						}
					>
						<ZoomInfo
							size="1.8rem"
							color={
								fieldsResult.filter(
									item =>
										item.backendField === "zoom_info" &&
										!item.retrievedValue &&
										item.value.name !== ""
								).length
									? Colors.redShade3 //what?
									: Colors.zoomInfo
							}
						/>
					</div> */}
				</div>
				<div className={styles.name}>
					<div className={styles.hoverContainer}>{Render("first_name")}</div>
					<div className={styles.hoverContainer}>{Render("last_name")}</div>
				</div>
				<div className={styles.companyInfo}>
					<Briefcase size="1rem" color={Colors.lightBlue} />{" "}
					<div className={`${styles.hoverContainer}`}>
						{Render("job_position", styles.jobPosition)}
					</div>{" "}
					at{" "}
					<div className={`${styles.hoverContainer}`}>
						{Render(
							(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.LEADS_ACCOUNTS) ||
								(!isOnboarding && current_view === VIEWS.LEAD)
								? "name"
								: "name",
							styles.companyName
						)}

						{Render(
							((isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.CANDIDATE) ||
								(!isOnboarding && current_view === VIEWS.CANDIDATE)) &&
								"company",
							styles.companyName
						)}
					</div>{" "}
					with <div className={styles.hoverContainer}>{Render("size")}</div> employees
				</div>

				<div className={styles.emailInfo}>
					<div className={styles.hoverContainer}>
						{(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.CANDIDATE) ||
						(!isOnboarding && current_view === VIEWS.CANDIDATE) ||
						(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.LEADS_ACCOUNTS) ||
						(!isOnboarding && current_view === VIEWS.LEAD) ? (
							<>
								{fieldsResult
									.filter(item => item.retrievedValue && item.backendField === "emails")
									?.map((it, index) => (
										<div className={styles.email} title={it.retrievedValue}>
											<Email color={Colors.lightBlue} />
											{it.retrievedValue}
											{index === 0 && <div></div>}
										</div>
									)) ??
									designField({
										text: "Email Fields",
										size: "1rem",
										color: Colors.redShade3,
									})}
								&nbsp;
								{fieldsResult.filter(
									item =>
										!item.retrievedValue &&
										item.value.name !== "" &&
										item.backendField === "emails"
								).length
									? designField({
											color: Colors.redShade3,
											text: `${
												fieldsResult.filter(
													item =>
														!item.retrievedValue &&
														item.value.name !== "" &&
														item.backendField === "emails"
												).length
											} Failed`,
									  })
									: ""}
							</>
						) : (
							// eslint-disable-next-line react/jsx-no-useless-fragment
							<>
								{currentView ? (
									<>
										{fieldsResult
											.filter(
												item => item.retrievedValue && item.backendField === "emails"
											)[0]
											?.retrievedValue?.map((it, index) => (
												<div className={styles.email} title={it.retrievedValue}>
													<Email color={Colors.lightBlue} />
													{it.email_id}
													{index === 0 && <div></div>}
												</div>
											)) ??
											designField({
												text: "Email Fields",
												size: "1rem",
												color: Colors.redShade3,
											})}
										&nbsp;
										{fieldsResult.filter(
											item =>
												!item.retrievedValue &&
												item.value.name !== "" &&
												item.backendField === "emails"
										).length
											? designField({
													color: Colors.redShade3,
													text: `${
														fieldsResult.filter(
															item =>
																!item.retrievedValue &&
																item.value.name !== "" &&
																item.backendField === "emails"
														).length
													} Failed`,
											  })
											: ""}
									</>
								) : (
									""
								)}
							</>
						)}
					</div>
				</div>
				<div className={styles.phoneInfo}>
					<div className={styles.hoverContainer}>
						{(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.CANDIDATE) ||
						(!isOnboarding && current_view === VIEWS.CANDIDATE) ||
						(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.LEADS_ACCOUNTS) ||
						(!isOnboarding && current_view === VIEWS.LEAD) ? (
							<>
								{fieldsResult
									.filter(
										item => item.retrievedValue && item.backendField === "phone_numbers"
									)
									?.map(it => (
										<div>
											<Home color={Colors.lightBlue} />
											{it.retrievedValue}
										</div>
									)) ??
									designField({
										text: "Phone numbers",
										size: "1rem",
										color: Colors.redShade3,
									})}
								&nbsp;
								{fieldsResult.filter(
									item =>
										!item.retrievedValue &&
										item.value.name !== "" &&
										item.backendField === "phone_numbers"
								).length
									? designField({
											text: `${
												fieldsResult.filter(
													item =>
														!item.retrievedValue &&
														item.value.name !== "" &&
														item.backendField === "phone_numbers"
												).length
											} Failed`,
											color: Colors.redShade3,
									  })
									: ""}
							</>
						) : (
							// eslint-disable-next-line react/jsx-no-useless-fragment
							<>
								{currentView && (
									<>
										{fieldsResult
											.filter(
												item =>
													item.retrievedValue && item.backendField === "phone_numbers"
											)[0]
											?.retrievedValue?.map(it => (
												<div>
													<Home color={Colors.lightBlue} />
													{it.phone_number}
												</div>
											)) ??
											designField({
												text: "Phone numbers",
												size: "1rem",
												color: Colors.redShade3,
											})}
										&nbsp;
										{fieldsResult.filter(
											item =>
												!item.retrievedValue &&
												item.value.name !== "" &&
												item.backendField === "phone_numbers"
										).length
											? designField({
													text: `${
														fieldsResult.filter(
															item =>
																!item.retrievedValue &&
																item.value.name !== "" &&
																item.backendField === "phone_numbers"
														).length
													} Failed`,
													color: Colors.redShade3,
											  })
											: ""}
									</>
								)}
							</>
						)}
					</div>
					<div className={styles.hoverContainer}>
						<Building color={Colors.lightBlue} />{" "}
						{Render(
							(isOnboarding && leadsCurrentView === LEADS_CURRENT_VIEWS.LEADS_ACCOUNTS) ||
								(!isOnboarding && current_view === VIEWS.LEAD)
								? "phone_number"
								: "phone_number"
						)}
					</div>
				</div>

				<div className={styles.location}>
					<Location size="1.1rem" />
					<div className={styles.hoverContainer}>{Render("zip_code")}</div>
					<div className={styles.hoverContainer}>{Render("country")}</div>
				</div>
			</div>
		</div>
	);
};

export default FetchedQuickView;
