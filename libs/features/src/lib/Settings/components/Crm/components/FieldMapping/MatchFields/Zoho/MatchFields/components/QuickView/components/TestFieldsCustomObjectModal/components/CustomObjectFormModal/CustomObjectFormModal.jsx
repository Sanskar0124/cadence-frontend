...
const CustomObjectFormModal = ({
		...
		fetchReferenceFieldOptions(body, {
			onSuccess: data => {
				setReferences(prev => ({
					...prev,
					[item.zoho_endpoint]: {
						...prev[item.zoho_endpoint],
						[item.zoho_field]: data.records,
					},
				}));
			},
			onError: err => {
	...
	const handleSave = () => {
		...
		form.forEach(item => {
			let value;
			if (item.zoho_endpoint === VIEWS.LEAD) {
				value = leadInp[item.zoho_field];
			} else if (item.zoho_endpoint === VIEWS.CONTACT) {
				value = contactInp[item.zoho_field];
			} else {
				value = accountInp[item.zoho_field];
			}

			if (item.type === "multi_select_dropdown") {
				if (item.zoho_endpoint === VIEWS.LEAD) {
					leadInp[item.zoho_field] = value.length === 0 ? null : value.join(";");
				} else if (item.zoho_endpoint === VIEWS.CONTACT) {
					contactInp[item.zoho_field] = value.length === 0 ? null : value.join(";");
				} else {
					accountInp[item.zoho_field] = value.length === 0 ? null : value.join(";");
				}
			} else if (item.type === "input_box") {
				const type = item.input_type;
				if (type === "date") {
					if (item.zoho_endpoint === VIEWS.LEAD) {
						leadInp[item.zoho_field] =
							value.DD !== "dd" ? `${value.YYYY}-${value.MM}-${value.DD}` : null;
					} else if (item.zoho_endpoint === VIEWS.CONTACT) {
						contactInp[item.zoho_field] =
							value.DD !== "dd" ? `${value.YYYY}-${value.MM}-${value.DD}` : null;
					} else {
						accountInp[item.zoho_field] =
							value.DD !== "dd" ? `${value.YYYY}-${value.MM}-${value.DD}` : null;
					}
				} else if (type === "datetime") {
					if (item.zoho_endpoint === VIEWS.LEAD) {
					...
					if (item.zoho_endpoint === VIEWS.LEAD) {
						leadInp[item.zoho_field] = value === "" ? null : value;
					} else if (item.zoho_endpoint === VIEWS.CONTACT) {
						contactInp[item.zoho_field] = value === "" ? null : value;
					} else {
						accountInp[item.zoho_field] = value === "" ? null : value;
					}
				}
			} else {
				if (item.zoho_endpoint === VIEWS.LEAD) {
					leadInp[item.zoho_field] = value === "" || value === undefined ? null : value;
				} else if (item.zoho_endpoint === VIEWS.CONTACT) {
					contactInp[item.zoho_field] =
		...
		if (buildFormFor === VIEWS.LEAD) {
			form.forEach(item => {
				if (item.editable === false) {
					delete leadInp[item.zoho_field];
				}
			});
			body = {
				type: VIEWS.LEAD,
			...
			form.forEach(item => {
				if (item.editable === false) {
					delete contactInp[item.zoho_field];
				}
			});
			body = {
				type: VIEWS.CONTACT,
			...
			form.forEach(item => {
				if (item.editable === false) {
					if (item.zoho_endpoint === VIEWS.CONTACT) delete contactInp[item.zoho_field];
					else delete accountInp[item.zoho_field];
				}
			});
			body = {
				type: VIEWS.CONTACT,
				...
				...(fields[VIEWS.CONTACT]?.Account_Name?.id && {
					custom_object_account: accountInp,
				}),

				...(fields[VIEWS.CONTACT]?.Account_Name?.id && {
					zoho_account_id:
						fields[VIEWS.CONTACT]?.Account_Name?.id ?? fields[VIEWS.ACCOUNT]?.id,
				}),
			};
		}
		...
		setTest(body, {
			onSuccess: () => {
				addSuccess(`Successfully updated ${buildFormFor} in zoho.`);
				onClose();
															...
															>
																...
																{item.type === "dropdown" && item.reference_to && (
																	<>
																		...
																		<Select
																			...
																			isLoading={
																				searchItem?.zoho_field === item?.zoho_field &&
																				fetchReferenceFieldOptionsLoading
																			}
																			name={item.zoho_field}
																			...
																			options={
																				Array.isArray(
																					...
																					: Object.keys(fields[item.zoho_endpoint])
																							.length !== 0 &&
																					  fields[item.zoho_endpoint][item.zoho_field] !=
																							null && [
																							{
																								label:
																...
																{item.type === "radio_button" && (
																	<div className={`${styles.radioBox}`}>
																		<div className={styles.label}>{item.zoho_label}?</div>
