...
const DynamicsFields = ({ availableSFFields, buildFormFor, loading }) => {
	const [searchValue, setSearchValue] = useState("");
	const [viewSFFieldsFor, setViewSFFieldsFor] = useState(
		buildFormFor === VIEWS.LEAD ? VIEWS.LEAD : VIEWS.ACCOUNT
	);
	console.log(availableSFFields, "Avl");
	return (
							...
							>
								{ALPHABETS.map((alph, j) => {
									return availableSFFields[viewSFFieldsFor]
										...
										?.filter(field => field.label[0].toUpperCase() === alph).length >
										0 ? (
