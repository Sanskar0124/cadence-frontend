...
const RingoverFields = ({
	...
	const checkDisabled = field => {
		return (
			(field.backendField === "zip_code" && field?.value?.name === "address.zipcode") ||
			(field.backendField === "country" && field?.value?.name === "address.countryName")
		);
	};
	...
	return (
		<div className={styles.ringoverFields}>
				...
				<div className={styles.rf} ref={outsideclickRef}>
					<div className={styles.row + " " + styles.title}>
						<div className={styles.col1}>
							{SETTINGS_TRANSLATION.BULLHORN_FIELDS[user?.language?.toUpperCase()]}
						</div>
							...
							<>
								{i === CUSTOM_FIELDS_HEADING[currentView] && (
									<div className={styles.row + " " + styles.title}>
										<div className={styles.col1}>
											{
												SETTINGS_TRANSLATION.BULLHORN_FIELDS[
													user?.language?.toUpperCase()
												]
											}
										</div>
			...
			<IntegrationStatusModal
				...
				isLeadContactAccountStatusMapped={isLeadContactAccountStatusMapped}
				defaultIntStatus={defaultIntStatus}
				setDefaultIntStatus={setDefaultIntStatus}
				...
				setDisqualifcationReason={val => {
					setFields(fields?.map(f => (f.uid === "__disqualification_reasons" ? val : f)));
				}}
				disqualificationReason={fields?.find(f => f.uid === "__disqualification_reasons")}
