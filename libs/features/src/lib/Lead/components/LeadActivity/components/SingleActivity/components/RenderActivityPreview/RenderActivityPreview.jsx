...
//constants
// oof stands for out of facility(out of office)
import { CUSTOM_TASK_NAME } from "./constants";
import styles from "./RenderActivityPreview.module.scss";

const RenderActivityPreview = ({ activity, handleActivityActions, cadence }) => {
		...
		switch (activity?.type) {
			...
			case "note":
				return (
					<div className={`${styles.activityPreview} ${styles?.[activity?.type]}`}>
						<div className={styles.title}>
							{activity?.Lead || activity?.User ? (
								<>
									Note for {activity?.Lead?.full_name ?? "Deleted Lead"} by{" "}
									{activity?.User?.first_name ?? "Deleted User"}{" "}
