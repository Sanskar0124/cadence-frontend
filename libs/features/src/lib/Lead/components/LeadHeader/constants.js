...
export const ACCOUNT_STAGES = {
	fake: "Fake",
	hors_cible: "Out of Focus",
	non_qualifie: "Pool",
	a_contacter: "To Be Qualified",
	en_attente: "Working",
	besoin_analyse: "Needs Analysis",
	interesse: "Interested",
};

export const LEAD_STAGES = {
	Unqualified: "Disqualified",
	"0.Nurturing": "Marketing Qualification",
	ToContact: "New",
	working: "Working",
	Pending: "Interested",
...
export const VISIBLE_BUTTONS = {
	ACCOUNT_STATUS: {
		[INTEGRATION_TYPE.SALESFORCE]: true,
		[INTEGRATION_TYPE.PIPEDRIVE]: false,
		[INTEGRATION_TYPE.GOOGLE_SHEETS]: false,
		[INTEGRATION_TYPE.SHEETS]: false,
		[INTEGRATION_TYPE.HUBSPOT]: true,
		[INTEGRATION_TYPE.SELLSY]: false,
	},
	QUALIFICATIONS: {
		[INTEGRATION_TYPE.SALESFORCE]: true,
		[INTEGRATION_TYPE.PIPEDRIVE]: true,
		[INTEGRATION_TYPE.GOOGLE_SHEETS]: false,
		[INTEGRATION_TYPE.SHEETS]: false,
		[INTEGRATION_TYPE.HUBSPOT]: false,
		[INTEGRATION_TYPE.SELLSY]: true,
		[INTEGRATION_TYPE.ZOHO]: false,
	},
	CUSTOM_TASK: {
		[INTEGRATION_TYPE.SALESFORCE]: true,
		[INTEGRATION_TYPE.PIPEDRIVE]: true,
		[INTEGRATION_TYPE.GOOGLE_SHEETS]: true,
		[INTEGRATION_TYPE.SHEETS]: true,
		[INTEGRATION_TYPE.HUBSPOT]: true,
		[INTEGRATION_TYPE.SELLSY]: true,
	},
	ADD_NOTE: {
	...
	switch (integration_type) {
		...
		case INTEGRATION_TYPE.BULLHORN:
			return lead?.integration_type === LEAD_INTEGRATION_TYPES.BULLHORN_CONTACT
				...
				? Boolean(
						fieldMap?.Company_Setting?.Integration_Field_Map?.candidate_map
							?.integration_status?.name
				  )
				: true;
		default:
...
const buttonFormSetBullhorn = ({ lead, fieldMap, setButtonText, setForm }) => {
	if (
		...
		if (fieldMap.Company_Setting.Integration_Field_Map?.candidate_custom_object != null) {
			setButtonText(
		...
		body = {
			...
			properties: {
				contact_properties: `${fieldMap.Company_Setting.Integration_Field_Map.contact_custom_object?.[0]?.form
					...
					.concat(
						fieldMap.Company_Setting.Integration_Field_Map.contact_custom_object?.[0]?.form?.filter(
							i => i?.integration_endpoint === VIEWS_HUBSPOT.CONTACT
						).length === 0
							? "associatedcompanyid"
							: ",associatedcompanyid"
					)}${
...
export const INTEGRATION_MODALS = {};
