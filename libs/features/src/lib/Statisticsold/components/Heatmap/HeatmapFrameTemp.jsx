...
const HeatmapFrameTemp = ({ timerangefilter, cadenceSelected, userfilter }) => {
	...
	const { heatmapDataLoading, heatmapData, setHeatmapFilter, heatmapHasError } =
		useHeatmapOld(filters, role);

	useEffect(() => {
		setYLabels(
			WEEKDAYS_LABEL?.map(item =>
				makeFirstLetterCapital(WEEK_DAYS[item][user?.language.toUpperCase()])
			)
		);
	}, [user?.language]);
...
export default HeatmapFrameTemp;
