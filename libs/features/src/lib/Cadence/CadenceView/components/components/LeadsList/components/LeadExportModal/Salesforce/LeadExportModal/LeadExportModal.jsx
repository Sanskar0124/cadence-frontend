...
const LeadExportModal = ({ modal, setModal, refetch }) => {
		...
		switch (exportAs) {
			case EXPORT_AS.LEAD: {
				...
				exportLead(
					...
					{
						onSuccess: () => {
							addSuccess(`Successfully exported lead as Salesforce lead`);
			...
			case EXPORT_AS.CONTACT: {
				...
				exportContact(
					...
					{
						onSuccess: () => {
							addSuccess(`Successfully exported lead as Salesforce contact`);
							if (refetch && typeof refetch === "function") refetch();
							onClose();
						},
						onError: err => {
	...
	return (
		<Modal
			...
			disableOutsideClick
		>
			<div className={styles.headingText}>Export to Salesforce</div>
