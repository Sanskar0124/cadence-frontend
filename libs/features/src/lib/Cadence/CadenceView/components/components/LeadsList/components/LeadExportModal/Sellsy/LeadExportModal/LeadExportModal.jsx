...
import { useLeadExportSellsy } from "@cadence-frontend/data-access";
...
const LeadExportModal = ({ modal, setModal, refetch }) => {
	...
	const [sellsyCompanies, setSellsyCompanies] = useState([]);
	...
	const handleSave = () => {
		...
		const last_name_label =
			contactForm.find(field => field.name === "last_name")?.integration_label ??
			"Lastname";
		if (contactInput.last_name === null || contactInput.last_name?.length === 0)
			return addError(`'${last_name_label}' for contact is required!`);

		const company_name_label =
			companyForm.find(field => field.name === "name")?.integration_label ?? "Name";
		if (companyInput?.name?.name === undefined || companyInput?.name?.name?.length === 0)
		...
		let companyData = { ...companyInput };
		const searchedCompanyData = companyData?.name ?? {};
		delete companyData?.name;
		body.company_data = { ...searchedCompanyData, ...companyData };
		...
		exportContact(body, {
			onSuccess: () => {
				addSuccess(`Successfully exported lead as Sellsy contact`);
	...
	useEffect(() => {
		if (seachCompany) {
			const timeoutId = setTimeout(() => {
				let body = { name: seachCompany };
				searchSellsyCompanies(body, {
					onSuccess: data => {
						let options = [];
						if (data?.length)
							data?.forEach(item => {
							...
							options.push({
								label: body.name,
								value: { ...companyInput, name: body.name },
								search: body.name.toLowerCase(),
							});
						setDefaultNewOption(false);
						setSellsyCompanies(options);
						if (options?.length)
							setCompanyInput(prev => ({
								...prev,
	...
	return (
		<Modal
			...
			disableOutsideClick
		>
			<div className={styles.headingText}>Export to Sellsy</div>
									...
									(item.type === "advanced_dropdown" ? (
										<div className={styles.inputGroup}>
											...
											<AdvanceSelect
												...
												options={sellsyCompanies}
												setOptions={setSellsyCompanies}
