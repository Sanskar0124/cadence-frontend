...
import { VIEWS, REFERENCES } from "./constants";
...
const QualificationsModal = ({
	...
	const handleInputChange = (input, item) => {
		let body = {
			bullhorn_endpoint: item.reference_field_name.bullhorn_endpoint,
	...
	const handleSave = () => {
		...
		if (lead.integration_type === LEAD_INTEGRATION_TYPES.BULLHORN_CANDIDATE) {
			...
			body = {
				...
				id: lead.integration_id.toString(),
			...
			if (
				...
				if (fields[VIEWS.LEAD]?.clientCorporation?.id) {
					body = {
						...
						bullhorn_corporation_id: fields[VIEWS.LEAD]?.clientCorporation.id.toString(),
					};
				} else {
					body = {
						type: VIEWS.LEAD,
				...
				form.forEach(item => {
					if (item.editable) {
						if (item.bullhorn_endpoint === VIEWS.CONTACT || VIEWS.CLIENT_CONTACT)
							delete contactInp[item.bullhorn_field];
				...
				body = {
					...
					bullhorn_corporation_id:
						fields[VIEWS.CONTACT]?.clientCorporation?.id.toString(),
				};
			}
		}
