...
import { VIEWS, PHONE_OPTIONS, EMAIL_OPTIONS } from "./constants";
...
const QualificationsModal = ({
	...
	const renderInput = (item, row) => {
		...
		switch (type) {
			...
			// 				value={
			// 					item.integration_endpoint === VIEWS.PERSON ? personInput : organizationInput
			// 				}
			// 				className={`${!item.editable && styles.disabled}`}
			// 				disabled={!item.editable}
			// 				setValue={
	...
	const handleSave = () => {
		...
		form.forEach(item => {
			...
			// } else
			if (item.integration_endpoint === VIEWS.PERSON) {
				value = personInp[item.integration_field];
			} else {
				value = organizationInp[item.integration_field];
			}
			...
			if (item.type === "input_box") {
				...
				// 	// } else
				// 	if (item.integration_endpoint === VIEWS.PERSON) {
				// 		personInp[item.integration_field] =
				// 			value.DD !== "dd" ? `${value.YYYY}-${value.MM}-${value.DD}` : null;
				// 	} else {
				// 		organizationInp[item.integration_field] =
				// 			value.DD !== "dd" ? `${value.YYYY}-${value.MM}-${value.DD}` : null;
				// 	}
				// }
				//  else
				if (type === "date") {
					...
					// }
					if (item.integration_endpoint === VIEWS.PERSON) {
						personInp[item.integration_field] = value === "" ? null : value;
					} else {
						organizationInp[item.integration_field] = value === "" ? null : value;
					}
				}
			} else if (item.type === "input_select") {
				if (item.integration_endpoint === VIEWS.PERSON) {
					personInp[item.integration_field] = value;
				} else {
					organizationInp[item.integration_field] = value;
				}
			} else {
				// if (item.integration_endpoint === VIEWS.LEAD) {
				// 	leadInp[item.salesforce_field] =
		...
		if (Object.keys(organizationInput).length === 0) {
			form.forEach(item => {
				if (item.editable === false) {
					delete personInp[item.integration_field];
				}
			});
			body = {
			...
			form.forEach(item => {
				if (item.editable === false) {
					delete organizationInp[item.integration_field];
				}
			});
			body = {
			...
			form.forEach(item => {
				if (item.editable === false) {
					if (item.integration_endpoint === VIEWS.PERSON)
						delete personInp[item.integration_field];
					else delete organizationInp[item.integration_field];
				}
			});
			body = {
				lead_id: lead?.lead_id.toString(),
	...
	// console.log(personInput, "Persom");
	// console.log(organizationInput, "Organi");
		...
		>
			...
			{lead?.account_id === null && (
				<div className={styles.note}>
					{" "}
					Fields related to organisation have been hidden as the contact has no such
					association
				</div>
			)}
															...
															{item.type === "input_select" && (
																<>
																	...
																	<div className={styles.inputSelectBox}>
																		{item.integration_endpoint === VIEWS.PERSON
																			? personInput[item.integration_field]?.map(
																					(inp, key) => {
																						return (
																							<div className={styles.current}>
																								...
																								{personInput[item.integration_field]
																									.length > 1 && (
																									<div
																			...
																			: organizationInput[item.integration_field]?.map(
																					(inp, key) => {
																						return (
																							<div className={styles.current}>
																								<div className={styles.inputBox}>
